...
selected = option_menu(
       menu_title=None, # wymagane, None- nie będzie go
       options = ['Ankieta', 'Staty'], # wymagane
       menu_icon = 'cast',
       default_index=0, # strona od ktorej zaczniemy
...
if selected == 'Ankieta':
    ...
    if st.button("Submit"):
        ...
        st.success(result)
        message = st.success("Questionnaire is correct")
...
if selected == 'Staty':
    ...
    if data is not None:
        ...
        all_columns_names = list(df.select_dtypes(include=[np.number]).columns.values) 
